...
public class FeistelCipher {
	...
	public String decode(String message) {
	    ...
	    for (int i = 0; i < paddedLength; i += 8) {
	        byte[] block = Arrays.copyOfRange(padded, i, i + 8);
	        byte[] decodedBlock = feistelBlockInverse(block);
	        System.arraycopy(decodedBlock, 0, result, i, 8);
	    }

	    return new String(result, StandardCharsets.UTF_8).replaceAll("\\x00+$", "");
	}

	// Versión inversa de feistelBlock
	private byte[] feistelBlockInverse(byte[] block) {
	    int left = ((block[0] & 0xFF) << 24) | ((block[1] & 0xFF) << 16) | ((block[2] & 0xFF) << 8) | (block[3] & 0xFF);
	    int right = ((block[4] & 0xFF) << 24) | ((block[5] & 0xFF) << 16) | ((block[6] & 0xFF) << 8) | (block[7] & 0xFF);
	    int keySum = key.chars().sum() % (1 << 32);

	    // aplicar rondas en orden inverso
	    int temp = left;
	    left = right;
	    right = temp;
